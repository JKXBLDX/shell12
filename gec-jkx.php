...
// ═══════════════════════════════════════════════════════════════════
//  LOGIN GATE — CMS-style authentication
//  Kredensial disimpan sebagai hash bcrypt di bawah (bukan plaintext).
// ═══════════════════════════════════════════════════════════════════
// Kredensial disimpan dalam bentuk hash bcrypt (bukan plaintext).
// Untuk mengganti user/pass, generate hash baru lalu tempel di sini:
//   php -r "echo password_hash('USERNAME', PASSWORD_DEFAULT);"
//   php -r "echo password_hash('PASSWORD', PASSWORD_DEFAULT);"
$GECKO_AUTH_USER_HASH = '$2y$12$LgjVztM51StJICNCzH2G8.Yt5MMlsroWTOD0GZhJNaKw7F1Dxt4DG';
$GECKO_AUTH_PASS_HASH = '$2y$12$Q.EH2vltXjJCw71kYxQK9eoqOcngMyc17fgsl5/bYM5dInIrd0O1W';

// Polyfill hash_equals untuk PHP < 5.6 (kompatibilitas lintas versi)
...
// Verifikasi kredensial terhadap hash (password_verify + fallback crypt untuk PHP < 5.5)
if (!function_exists('gecko_verify_hash')) {
    function gecko_verify_hash($plain, $hash)
    {
        if (function_exists('password_verify')) {
            return password_verify((string)$plain, (string)$hash);
        }
        $check = @crypt((string)$plain, (string)$hash);
        return is_string($check) && $check !== '' && hash_equals($check, (string)$hash);
    }
}

// Mulai session (dengan fallback cookie bertanda tangan bila session gagal)
if (session_status() === PHP_SESSION_NONE) {
    @session_start();
}
$geckoAuthKey = 'gecko_' . substr(md5(__FILE__), 0, 8);

function geckoAuthToken()
{
    $secret = $GLOBALS['GECKO_AUTH_USER_HASH'] . '|' . $GLOBALS['GECKO_AUTH_PASS_HASH'];
    return hash_hmac('sha256', 'gecko-auth-v1', $secret);
}

function geckoIsAuthed()
{
    $key = $GLOBALS['geckoAuthKey'];
    if (isset($_SESSION[$key]) && $_SESSION[$key] === true) return true;
    if (isset($_COOKIE[$key])) {
        return hash_equals(geckoAuthToken(), (string)$_COOKIE[$key]);
    }
    return false;
}

// ── Logout ──────────────────────────────────────────────────────
if (isset($_GET['gecko_logout'])) {
    $key = $GLOBALS['geckoAuthKey'];
    unset($_SESSION[$key]);
...
// ── Proses login ────────────────────────────────────────────────
$geckoAuthError = '';
if (isset($_POST['gecko_user'])) {
    $u = (string)$_POST['gecko_user'];
    $p = isset($_POST['gecko_pass']) ? (string)$_POST['gecko_pass'] : '';
    if (gecko_verify_hash($u, $GECKO_AUTH_USER_HASH) && gecko_verify_hash($p, $GECKO_AUTH_PASS_HASH)) {
        $key = $GLOBALS['geckoAuthKey'];
        $_SESSION[$key] = true;
        if (session_status() === PHP_SESSION_ACTIVE) {
            @session_regenerate_id(true);
        }
        setcookie($key, geckoAuthToken(), time() + (86400 * 7), '/', '', !empty($_SERVER['HTTPS']), true);
        header('Location: ' . strtok($_SERVER['REQUEST_URI'], '?'));
        exit;
    }
    $geckoAuthError = 'Username atau password salah.';
    usleep(400000);
}

// ── Gate: blokir akses jika belum login ────────────────────────
if (!geckoIsAuthed()) {
    // Request API (JSON) → kembalikan 401 Unauthorized
    if (isset($_GET['api'])) {
        while (ob_get_level() > 0) ob_end_clean();
        http_response_code(401);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(array('ok' => false, 'error' => 'Unauthorized', 'auth' => false));
        exit;
    }

    echo <<<'GECKO_LOGIN_HTML'
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="utf-8">
...
<style>
:root{--bg:#0f1115;--card:#171a21;--bd:#232838;--tx:#e6e8ee;--tx2:#8b93a7;--acc:#4f7cff;--acc2:#3b5fd9;--red:#f87171;--in:#12151c}
*{box-sizing:border-box;margin:0;padding:0}
body{min-height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(1200px 600px at 50% -10%,#1a2030 0%,#0f1115 60%);font-family:'Segoe UI',system-ui,-apple-system,sans-serif;color:var(--tx);padding:20px}
.box{width:100%;max-width:380px;background:var(--card);border:1px solid var(--bd);border-radius:16px;padding:34px 30px;box-shadow:0 20px 60px rgba(0,0,0,.45)}
.logo{display:flex;align-items:center;justify-content:center;width:56px;height:56px;margin:0 auto 16px;border-radius:14px;background:linear-gradient(135deg,var(--acc),#7c5cff);font-size:26px;font-weight:700;color:#fff}
h1{text-align:center;font-size:20px;font-weight:600;margin-bottom:4px}
.sub{text-align:center;color:var(--tx2);font-size:13px;margin-bottom:24px}
label{display:block;font-size:12px;font-weight:600;color:var(--tx2);margin:0 0 6px 2px;text-transform:uppercase;letter-spacing:.4px}
.inp{width:100%;padding:12px 14px;background:var(--in);border:1px solid var(--bd);border-radius:10px;color:var(--tx);font-size:14px;margin-bottom:16px;outline:none;transition:.15s}
.inp:focus{border-color:var(--acc);box-shadow:0 0 0 3px rgba(79,124,255,.15)}
.btn{width:100%;padding:12px;border:0;border-radius:10px;background:linear-gradient(135deg,var(--acc),var(--acc2));color:#fff;font-size:14px;font-weight:600;cursor:pointer;transition:.15s}
.btn:hover{filter:brightness(1.08)}
.err{background:rgba(248,113,113,.12);border:1px solid rgba(248,113,113,.35);color:var(--red);font-size:13px;padding:10px 12px;border-radius:10px;margin-bottom:16px}
.foot{text-align:center;color:var(--tx2);font-size:11px;margin-top:20px}
</style>
</head>
<body>
...
GECKO_LOGIN_HTML;
    ...
    if ($geckoAuthError !== '') {
        echo '<div class="err">' . htmlspecialchars($geckoAuthError, ENT_QUOTES, 'UTF-8') . '</div>';
    }

    echo <<<'GECKO_LOGIN_HTML'
  <label for="u">Username</label>
  <input class="inp" type="text" id="u" name="gecko_user" placeholder="Username" required autofocus>
  <label for="p">Password</label>
  <input class="inp" type="password" id="p" name="gecko_pass" placeholder="Password" required>
  <button class="btn" type="submit">Masuk</button>
  <p class="foot">Protected area &middot; Authorized access only</p>
</form>
</body>
</html>
GECKO_LOGIN_HTML;
...
<header class="topbar">
  ...
  <div class="topbar-right">
    ...
    <a class="btn btn-term" href="?gecko_logout=1" title="Logout">
      <svg viewBox="0 0 16 16"><path d="M6 2.75A.75.75 0 0 1 6.75 2h6.5a.75.75 0 0 1 .75.75v10.5a.75.75 0 0 1-.75.75h-6.5a.75.75 0 0 1 0-1.5h5.75V3.5H6.75A.75.75 0 0 1 6 2.75Zm3.22 3.47a.75.75 0 0 1 1.06 0l1.25 1.25a.75.75 0 0 1 0 1.06l-1.25 1.25a.75.75 0 1 1-1.06-1.06l.47-.47H3.75a.75.75 0 0 1 0-1.5h5.94l-.47-.47a.75.75 0 0 1 0-1.06Z"/></svg>
      Logout
    </a>
  </div>
</header>
